...
async function SingleProductPage({
  ...
  const product = await fetchSingleProduct(awaitedParams.id);
        ...
        <div>
          <div className="flex gap-x-8 items-center">
            ...
            <FavoriteToggleButton
              isFeatured={awaitedParams.isFeatured || false}
              productId={awaitedParams.id}
            />
          </div>
          <ProductRating productId={awaitedParams.id} />
          <h4 className="text-xl mt-2">
            Seller:{" "}
            <Link href={`/profile/${seller.username}`}>
              <span className="cursor-pointer text-blue-800">
          ...
          <p className="mt-6 leading-8 text-muted-foreground">{description}</p>
          <AddToCart bookId={awaitedParams.id} />
        </div>
      </div>
    </section>
  );
