...
function SellerProducts() {
  // Define all the state variables at the top level
  const [items, setItems] = useState<any[]>([]); // Ensuring items is an array
  const [error, setError] = useState("");
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      ...
      if (!sellerId) {
        setError("Seller ID not found in local storage.");
        setLoading(false); // Stop loading if no seller ID
        return;
      }
    ...
    fetchProducts(); // Initial fetch on component mount
  }, []); // Empty dependency array, ensuring this runs only once on mount

  // Ensure loading state and error are checked first, before rendering the table
  if (loading) {
